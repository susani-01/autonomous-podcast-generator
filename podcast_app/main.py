...
class ArticleRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"text": "Enter your article here", "music": True}
        }
...
def generate(request: ArticleRequest):
    ...
    task = generate_podcast.delay(
        input_text=request.text, url=request.url, music=request.music
    )
    return JSONResponse(
        {
            "task_id": task.id,
...
def download_podcast(task_id: str):
    ...
    if task.state != "SUCCESS":
        raise HTTPException(
            status_code=400, detail=f"Podcast not ready yet. Status: {task.state}"
        )
...
def get_script(task_id: str):
    ...
    return {"task_id": task_id, "script": task.result["script"]}


@app.get("/health")
def health():
    return {"status": "healthy", "redis": "connected", "version": "1.0.0"}
